...
export function OrganizationRuleSettingsCard({ organizationId }: { organizationId: string }) {
  const query = useOrganizationRuleSettingsQuery(organizationId, Boolean(organizationId));
    ...
    <Card className="space-y-4">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Adjust the thresholds the backend rule engine uses to detect issues for this organization. High-severity
          thresholds still use the platform default.
        </p>
      </div>
      ...
      {mutation.isError ? (
        <ErrorState title="Could not save rule settings" message={getErrorMessage(mutation.error)} />
      ) : null}
      ...
      {draft ? (
        <div className="space-y-4">
          <RuleSection
            ...
            onToggle={(enabled) => setDraft({ ...draft, largePR: { ...draft.largePR, enabled } })}
          >
            <NumberField
              label="Files threshold"
              value={draft.largePR.filesThreshold}
              onChange={(filesThreshold) => setDraft({ ...draft, largePR: { ...draft.largePR, filesThreshold } })}
            />
            <NumberField
              label="Total changes threshold"
              value={draft.largePR.totalChangesThreshold}
              onChange={(totalChangesThreshold) =>
                setDraft({ ...draft, largePR: { ...draft.largePR, totalChangesThreshold } })
              }
            />
          </RuleSection>
          ...
          <RuleSection
            ...
            onToggle={(enabled) => setDraft({ ...draft, slowReview: { ...draft.slowReview, enabled } })}
          >
            <NumberField
              label="Wait hours threshold"
              value={draft.slowReview.waitHoursThreshold}
              onChange={(waitHoursThreshold) =>
                setDraft({ ...draft, slowReview: { ...draft.slowReview, waitHoursThreshold } })
              }
            />
          </RuleSection>
          ...
          <RuleSection
            ...
            onToggle={(enabled) => setDraft({ ...draft, hotspot: { ...draft.hotspot, enabled } })}
          >
            <NumberField
              label="Score threshold"
              value={draft.hotspot.scoreThreshold}
              onChange={(scoreThreshold) => setDraft({ ...draft, hotspot: { ...draft.hotspot, scoreThreshold } })}
            />
          </RuleSection>
          ...
          <RuleSection
            ...
            onToggle={(enabled) =>
              setDraft({ ...draft, deploymentFailure: { ...draft.deploymentFailure, enabled } })
            }
          >
            <NumberField
              ...
              onChange={(minimumDeployments) =>
                setDraft({ ...draft, deploymentFailure: { ...draft.deploymentFailure, minimumDeployments } })
              }
            />
            <NumberField
              ...
              onChange={(failureRateThreshold) =>
                setDraft({ ...draft, deploymentFailure: { ...draft.deploymentFailure, failureRateThreshold } })
              }
            />
          </RuleSection>
          ...
          <RuleSection
            ...
            onToggle={(enabled) =>
              setDraft({ ...draft, reviewConcentration: { ...draft.reviewConcentration, enabled } })
            }
          >
            <NumberField
              ...
              onChange={(minimumReviewCount) =>
                setDraft({ ...draft, reviewConcentration: { ...draft.reviewConcentration, minimumReviewCount } })
              }
            />
            <NumberField
              ...
              onChange={(shareThreshold) =>
                setDraft({ ...draft, reviewConcentration: { ...draft.reviewConcentration, shareThreshold } })
              }
            />
          </RuleSection>
          ...
          <RuleSection
            ...
            onToggle={(enabled) => setDraft({ ...draft, bottleneck: { ...draft.bottleneck, enabled } })}
          >
            <NumberField
              label="Minimum merged count"
              value={draft.bottleneck.minimumMergedCount}
              onChange={(minimumMergedCount) =>
                setDraft({ ...draft, bottleneck: { ...draft.bottleneck, minimumMergedCount } })
              }
            />
            <NumberField
              ...
              onChange={(averageCycleHoursThreshold) =>
                setDraft({ ...draft, bottleneck: { ...draft.bottleneck, averageCycleHoursThreshold } })
              }
            />
            <NumberField
              ...
              onChange={(staleOpenCountThreshold) =>
                setDraft({ ...draft, bottleneck: { ...draft.bottleneck, staleOpenCountThreshold } })
              }
            />
            <NumberField
              ...
              onChange={(staleOpenAgeDays) =>
                setDraft({ ...draft, bottleneck: { ...draft.bottleneck, staleOpenAgeDays } })
              }
            />
          </RuleSection>
          ...
          <div className="space-y-3 rounded-xl border border-border/70 bg-background/70 p-4">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <label className="space-y-1">
                <span className="text-xs text-muted-foreground">Default day type</span>
                <Select
                  ...
                  onChange={(event) =>
                    setDraft({
                      ...draft,
                      metrics: { ...draft.metrics, defaultDayType: event.target.value as "calendar" | "business" },
                    })
                  }
                >
              ...
              <NumberField
                ...
                onChange={(hotspotCommitWeight) =>
                  setDraft({ ...draft, metrics: { ...draft.metrics, hotspotCommitWeight } })
                }
              />
              <NumberField
                ...
                onChange={(hotspotAdditionsWeight) =>
                  setDraft({ ...draft, metrics: { ...draft.metrics, hotspotAdditionsWeight } })
                }
              />
              <NumberField
                ...
                onChange={(hotspotDeletionsWeight) =>
                  setDraft({ ...draft, metrics: { ...draft.metrics, hotspotDeletionsWeight } })
                }
              />
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Button
              ...
              onClick={() => draft && mutation.mutate({ organizationId, payload: draft })}
            >
